...
read.table(paste0(prefix,".mad"),header=T)->mad
mad[mad$Mgt=="0/1",]->mad.het
mad.het$Malt/(mad.het$Mref+mad.het$Malt)->mad.het$ratio
boxplot.stats(mad.het$ratio)->mad.het.box
mad.het[!mad.het$ratio%in%mad.het.box$out,]->mad.het.fix
mean(mad.het.fix$ratio)->mad.het.mean
sd(mad.het.fix$ratio)->mad.het.sd
print(paste(mad.het.mean,mad.het.sd))
#e_ratio <-0.5
e_ratio <- mad.het.mean


read.table(paste0(prefix,".pat_origin.hap"),stringsAsFactors=F,header=T)->OUT
OUT[OUT$M0!=OUT$M1,]->md.fix
uplimit=0.95
lowlimit=0.05


if(1){
    for(i in 1:nrow(md.fix)){
	#chr_ratio[md.fix$chr[i]]->e_ratio
        Mp_dep=md.fix$Mp_ref[i]+md.fix$Mp_alt[i]
        if(md.fix$F_origin[i]==0){
            dbinom(md.fix$Mp_alt[i],Mp_dep,(1-fra.mean)*e_ratio)->md.fix$Hom[i]
            dbinom(md.fix$Mp_alt[i],Mp_dep,e_ratio)->md.fix$Het[i]
	    if(md.fix$M0[i]==0){ # Hom
		md.fix$Hom[i]/(md.fix$Hom[i]+md.fix$Het[i])->md.fix$P0[i]
	    }else{ # Het
		md.fix$Het[i]/(md.fix$Hom[i]+md.fix$Het[i])->md.fix$P0[i]
	    }
        }else if(md.fix$F_origin[i]==1){
            dbinom(md.fix$Mp_alt[i],Mp_dep,(1-fra.mean)*e_ratio+fra.mean)->md.fix$Hom[i]
            dbinom(md.fix$Mp_alt[i],Mp_dep,e_ratio)->md.fix$Het[i]
	    if(md.fix$M0[i]==1){ # Hom
		md.fix$Hom[i]/(md.fix$Hom[i]+md.fix$Het[i])->md.fix$P0[i]
	    }else{ # Het
		md.fix$Het[i]/(md.fix$Hom[i]+md.fix$Het[i])->md.fix$P0[i]
	    }
        }
        md.fix$P0[i]=min(md.fix$P0[i],uplimit)
        md.fix$P0[i]=max(md.fix$P0[i],lowlimit)
        1-md.fix$P0[i]->md.fix$P1[i]
    }
}
...
for(i in 1:length(chr)){
    if(nrow(md.fix[md.fix$chr==chr[i],])>1){
        md.fix[md.fix$chr==chr[i],]->t
        1:nrow(t)->Symbols
        ...
        t$fetal->md.fix[md.fix$chr==chr[i],"fetal_M"]
		t$or->md.fix[md.fix$chr==chr[i],"OddRatio_M"]
    }
}
write.table(md.fix,paste0(prefix,".OR.mout"),sep="\t",quote=F,row.names=F)
