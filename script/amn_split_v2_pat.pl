...
while(<IN>){
	...
	$amn_F=$ln[$headers{"amn_F"}];
	$amn_M=$ln[$headers{"amn_M"}];
    $ln[0]=~s/chr//g;
    $Chr=$ln[0];
    $Pos=$ln[1];
    $P0=$ln[-3];
    ...
    for my$region(@Region){
        $fetal_M and print MOUT "$Chr\t$Pos\t$P0\t$P1\t$fetal_M\t$amn_F\t$region\n";
    }
}
close IN;
close MOUT;
say "Rscript $Plot_R $md.fout.plot $region $Gene";
system"Rscript $Plot_R $md.fout.plot $region $Gene";
sub getGENE(){
    my$pos=$_[0];
    my$region="";
...
=cut
cut -f 1,2,25,26,29,30,31,32 test3.ffout.xls |head
