...
if(-e"$prefix\_nipt.OR.mout"){
	...
	if(!exists$hashM{"fetal_M"} or $ar[$hashM{"fetal_M"}]==0){
	    if ($ar[$hashM{"P0"}]>0.5){
		$m0++;
	    }elsif($ar[$hashM{"P0"}]<0.5){
		$m1++;
	    }
	}elsif ($ar[$hashM{"fetal_M"}]==1){
	    $m0++;
...
if(-e"$prefix\_nipt.OR.fout"){
	...
	if(!exists$hashF{"fetal_F"} or $ar[$hashF{"fetal_F"}]==0){
	    if ($ar[$hashF{"P0"}]>0.5){
		$f0++;
	    }elsif($ar[$hashF{"P0"}]<0.5){
		$f1++;
	    }
	}elsif ($ar[$hashF{"fetal_F"}]==1){
	    $f0++;
